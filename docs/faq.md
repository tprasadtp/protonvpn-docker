...
Port forwarding is not supported directly, but the image includes tools required to setup as custom
script(`socat` and `natpmpc`) is required. It is being tracked via [#125](https://github.com/tprasadtp/protonvpn-docker/issues/125). It might be necessary to write your `service` loop which keeps port forwarding updated. Following commands can be used to setup VPN connection and check it regularly.
...
depends on protonwire running in background.
